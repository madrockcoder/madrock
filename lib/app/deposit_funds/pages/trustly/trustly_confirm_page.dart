...
class TrustlyConfirmDepositPage extends StatelessWidget {
  final TrustlyPaymentResponse paymentResponse;
  final Wallet wallet;
  const TrustlyConfirmDepositPage(
      {Key? key, required this.paymentResponse, required this.wallet})
      : super(key: key);

  static Future<void> show(BuildContext context, Wallet wallet,
      TrustlyPaymentResponse paymentResponse) async {
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => TrustlyConfirmDepositPage(
  ...
  Widget confirmButton(BuildContext context, TextTheme textTheme) {
    return Padding(
        ...
        child: CustomElevatedButtonAsync(
          onPressed: () async {
            await TrustlyViewModel().initiatePayment(
                context,
